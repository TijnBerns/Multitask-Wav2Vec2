...
from data.generate_merged_samples import PairGeneratorRepeatA, PairGeneratorRepeatB, PairGeneratorNoRepeat
import data.utils
import data.custom_datasets as custom_datasets
...
def merge_dataset(dataset, dataset_str: str, num_samples: int) -> None:
    ...
    pair_generator_repeatA = PairGeneratorRepeatA(num_samples=num_samples,
                                                  min_tokens=Config.min_tokens,
                                                  max_tokens=Config.max_tokens,
                                                  max_attempts=Config.max_attempts)

    pair_generator_repeatB = PairGeneratorRepeatB(num_samples=num_samples,
                                                  min_tokens=Config.min_tokens,
                                                  max_tokens=Config.max_tokens,
                                                  max_attempts=Config.max_attempts)
    ...
    for generator, suffix in [
        (pair_generator_repeatA, "-repA"),
        (pair_generator_repeatB, "-repB"),
...
def transcribe_libri_clean():
    ...
    for dataset_str, dataset in tqdm(custom_datasets.clean_datasets.items(), desc="Writing transcriptions for clean datasets"):
        data.utils.write_trans_clean(dataset, dataset_str, Config.datapath + "/LibriSpeech")

    trans_files = list(Path(Config.datapath).rglob("*trans.csv"))
...
def main(merge: bool, transcribe: bool, create_vocabs: bool):
    ...
    # Create vocab file
    if create_vocabs:
        data.utils.write_speaker_id_vocab(dataset=custom_datasets.clean_datasets["train-clean-100"],
